 ...
 * pages exist, all 486 manifest topics have HTML (and vice versa, no
...
describe('generated topic pages match the manifest', () => {
  it('generates all 486 topic pages with no orphans either way', () => {
    const { manifest } = loadDistManifest();
    assert.equal(manifest.topics.length, 486, 'dist manifest must hold 486 topics');
    const expected = new Set(
      manifest.topics.map((t) => path.join(DIST, t.courseCode, t.filename.replace(/\.md$/, '.html'))),
    );
    assert.equal(expected.size, 486, 'manifest must point at 486 unique HTML targets');
    ...
    assert.equal(onDisk.size, 486, `dist/ must hold 486 topic pages — actual: ${onDisk.size}`);
