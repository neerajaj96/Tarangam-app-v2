...
// and orphan findings are warnings only. Prints a concise report.
{
  ...
  if (graphSchema && curriculumDoc) {
    ...
    if (analysis.errors.length === 0) {
      ...
      if (manifest.aggregates.metadataTopics !== 415) {
        fail(`topic-manifest: expected 415 metadata topics (migration baseline) — actual: ${manifest.aggregates.metadataTopics}`);
      }
