...
// and orphan findings are warnings only. Prints a concise report.
{
  ...
  if (graphSchema && curriculumDoc) {
    ...
    if (analysis.errors.length === 0) {
      ...
      if (manifest.aggregates.metadataTopics !== 213) {
        fail(`topic-manifest: expected 213 metadata topics (migration baseline) — actual: ${manifest.aggregates.metadataTopics}`);
      }
