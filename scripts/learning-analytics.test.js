...
describe('live 486-topic repository', () => {
  ...
  it('scores the full curriculum deterministically', () => {
    ...
    assert.equal(a.totals.total, 486);
    assert.equal(a.courses.length, 20);
    assert.ok(a.studyTime.totalMinutes > 0);
    assert.ok(a.summary.includes('0 of 486 topics completed'));
    assert.equal(
    ...
    assert.equal(partial.summary, '216 of 486 topics completed — 44.4% curriculum coverage.');
  });
});
